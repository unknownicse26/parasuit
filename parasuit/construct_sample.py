...
class ConstructSample:
    ...
    def new_sampling(self, parameter_values, options_all, integer_option, float_option, datas, covs, score):
        self.ranges = dict()
        if "-make-concrete-symbolic" in options_all:
            options_all.remove("-make-concrete-symbolic")
        self.options_all = options_all
        k_ranges = []
        coverage = []
        n_option = len(options_all)
        ...
        for z in range(len(options_all)):
            ...
            self.ranges[options_all[z]] = sorted_data[select_k]

        return parameter_values, datas



    def after_clustering(self, parameter_values, score, b, option, integer_option, float_option, datas):
        self.avg_cov[b][self.select_K[b]] = (self.avg_cov[b][self.select_K[b]] * self.avg_count[b][
            self.select_K[b]] + score) // (self.avg_count[b][self.select_K[b]] + 1)
        self.avg_count[b][self.select_K[b]] += 1
        self.average = (self.average * self.iters + score) / (self.iters + 1)
        self.iters += 1
