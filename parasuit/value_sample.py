...
class Sampler:
    ...
    def sample_value(self, selected, coverage_log, initial_values, trials, threshold):
    ...
    def try_cluster(self, param, types, trials, threshold):
        # When there isn't enough data for clustering, apply the exploration policy
        if len(self.covered_data[param]) <= trials:
            return "explore"
