...
class PSConstruct:
    ...
    def solver(self, options, search_space, coverage, branches, iter, total_cov):
        if iter % 2 == 1:
            self.update_score(total_cov)

        search_space = self.integer(options[0], search_space, coverage)
        search_space = self.string(options[1], search_space, coverage)
        search_space = self.floats(options[2], search_space)

        return search_space


    def integer(self, options, search_space, value):
        for i in range(len(options)):
            ...
            if (self.i_i[i] % 2) == 1:
                ...
                if self.i_i[i] == 1:
                    self.i_n[i] = search_space['space'][option][0][-1]
                ...
                search_space['space'][option][0] = [self.i_num1[i]]
                ...
                search_space['space'][option][0] = [self.i_num2[i]]
                self.i_r1[i] = value
    ...
    def string(self, options, search_space, value):
        for i in range(len(options)):
            option = options[i]
            ...
            if (self.s_i[i] % 2) == 1:
                ...
                if self.s_i[i] == 1:
                    n = search_space['space'][option][0][-1]
                    idx = n.find('s')
                ...
                search_space['space'][option][0] = ["%ds" % (self.s_num1[i])]
                ...
                search_space['space'][option][0] = ["%ds" % (self.s_num2[i])]
                self.s_r1[i] = value
    ...
    def floats(self, options, search_space):
        for i in range(len(options)):
            option = options[i]
            new_value = rd.random()
    ...
    def option_seperator(self, search_space):
        key = list(search_space['space'].keys())
        for i in range(len(search_space['space'].keys())):
            option = key[i]
            if len(search_space['space'][option][0]) >= 4:
                continue

            if search_space['space'][option][0] == []:
                continue

            elif isinstance(search_space['space'][option][0][-1], int) and search_space['space'][option][0][-1] != 1:
                self.integer_option.append(option)
            elif (isinstance(search_space['space'][option][0][-1], str)) and search_space['space'][option][0][0][
                -1] == 's' and search_space['space'][option][0][0] not in self.not_string:
                self.string_option.append(option)
            elif (search_space['space'][option][0][-1] == 1.0):
    ...
    def put_init(self, search_space):
        ...
        for i in range(len(options_all)):
            if options_all[i] in self.string_option:
                n = search_space['space'][options_all[i]][0][-1]
                idx = n.find('s')
                self.datas[i].append(int(n[:idx]))
            else:
                self.datas[i].append(search_space['space'][options_all[i]][0][-1])
