...
class Filter:
    def __init__(self, pClass, parameter_values):
        ...
        self.keywords = ["write", "debug", "log", "pc", "print", "replay"]
        self.escape = ["pct"]
    ...
    def filtering(self, pClass):
        ...
        for b_opt in self.bool_opts:
            ...
            for key in self.keywords :
                if (key in b_opt) or (key in pClass.description[b_opt]):
                    filtered = 1
        ...
        for i_opt in self.int_opts:
            filtered = 0
            for key in self.keywords:
                if (key in i_opt) or (key in pClass.description[i_opt]):
                    filtered = 1
        ...
        for f_opt in self.float_opts:
            filtered = 0
            for key in self.keywords:
                if (key in f_opt) or (key in pClass.description[f_opt]):
                    filtered = 1
        ...
        for t_opt in self.time_opts:
            filtered = 0
            for key in self.keywords:
                if (key in t_opt) or (key in pClass.description[t_opt]):
                    filtered = 1
        ...
        for s_opt in self.str_opts:
            filtered = 0
            for key in self.keywords:
                if (key in s_opt) or (key in pClass.description[s_opt]):
                    filtered = 1
        ...
        print("[INFO] ParaSuit : Filtered Options : ", len(ft_list))
