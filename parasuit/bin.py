...
def main(argv=None):
    ...
    hyperparameters.add_argument('-p', '--parameter-values', default="initial_values.json", type=str, metavar='JSON',
    ...
    # Initialize ParaSuit
    parasuit = ParaSuit(args.budget, parameter_values, args.n_tune, args.clust_range, args.threshold)
    ...
    if args.tool == "parasuit":
        print('[INFO] ParaSuit : Selected ParaSuit. Parameters will be tuned.')
    elif args.tool == "klee":
        print('[INFO] ParaSuit : Selected default KLEE. Parameters will not be tuned.')
    ...
    while elapsed <= args.budget:
        ...
        if args.tool == "parasuit":
            parameter_values = parasuit.sample(elapsed, len(coverage), coverage_one, parameter_values, output_dir, run_dir, i)
