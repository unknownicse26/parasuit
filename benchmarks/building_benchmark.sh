...
function build_du-9.4 () {
    cd $BASE_DIRECTORY
    log INFO "Downloading: coreutils-9.4"
    download_source_tgz coreutils-9.4 https://ftp.gnu.org/gnu/coreutils/coreutils-9.4.tar.gz
    downloaded=$?
    if [ $downloaded -ne 0 ]; then
        log FAIL "Failed to build coreutils-9.4"
        return 1
    fi

    cd $BASE_DIRECTORY/coreutils-9.4
    log INFO "Build gcov object: du-9.4"
    build_multiple_gcov_obj obj-gcov src/du
    if [ $? -ne 0 ] ; then
        log FAIL "Failed to build gcov object: du-9.4"
    fi

    cd $BASE_DIRECTORY/coreutils-9.4
    log INFO "Build LLVM object: du-9.4"
    build_multiple_llvm_obj obj-llvm src/du.bc src
    if [ $? -ne 0 ] ; then
        log FAIL "Failed to build LLVM object: du-9.4"
    fi
    log INFO "Build process finished: du-9.4"
}
...
function build_ls-9.4 () {
    ...
    cd $BASE_DIRECTORY/coreutils-9.4
    log INFO "Build gcov object: ls-9.4"
    build_multiple_gcov_obj obj-gcov src/ls
    if [ $? -ne 0 ] ; then
        log FAIL "Failed to build gcov object: ls-9.4"
    fi

    cd $BASE_DIRECTORY/coreutils-9.4
    log INFO "Build LLVM object: ls-9.4"
    build_multiple_llvm_obj obj-llvm src/ls.bc src
    if [ $? -ne 0 ] ; then
        log FAIL "Failed to build LLVM object: ls-9.4"
    fi
    log INFO "Build process finished: ls-9.4"
}
...
Benchmark lists
    ...
    du-9.4         coreutils-9.4
    enscript-1.6.6
    gawk-5.1.0
    gcal-4.1
    grep-3.4
    ls-9.4         coreutils-9.4
    nano-4.9
    sed-4.8
    trueprint-5.4
    xorriso-1.5.2
...
if [ "$1" = "all" ] ; then
    benchmarks="combine-0.4.0 diff-3.7 du-9.4 enscript-1.6.6 gawk-5.1.0 gcal-4.1 grep-3.4 ls-9.4 nano-4.9 sed-4.8 trueprint-5.4 xorriso-1.5.2"
else
    benchmarks=$@
fi
